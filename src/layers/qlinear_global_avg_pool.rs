...
impl Layer for QLinearGlobalAvgPool {
    fn execute(&mut self, values: &HashMap<String, Tensor>, output: &mut Tensor) -> Result<()> {
        ...
        let x_scale = get_tensor(values, &self.inputs[1])?
            .floats()
            .context("in QLinearGlobalAvgPool layer")?[0];
        let x_zp = get_tensor(values, &self.inputs[2])?
            .floats()
            .context("in QLinearGlobalAvgPool layer")?[0];
        let y_scale = get_tensor(values, &self.inputs[3])?
            .floats()
            .context("in QLinearGlobalAvgPool layer")?[0];
        let y_zp = get_tensor(values, &self.inputs[4])?
            .floats()
            .context("in QLinearGlobalAvgPool layer")?[0];
        ...
        crate::layers::quantize_u8_into(
            self.gap_output
                .floats()
                .context("in QLinearGlobalAvgPool layer")?,
            y_scale,
            y_zp,
            out_buf,
        );
