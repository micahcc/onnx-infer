...
impl Layer for Lstm {
    fn execute(&mut self, values: &HashMap<String, Tensor>, output: &mut Tensor) -> Result<()> {
        ...
        let bias = if self.inputs.len() > 3 && !self.inputs[3].is_empty() {
            get_tensor(values, &self.inputs[3])?.floats().context("in Lstm layer")?
        } else {
            &zero_bias
        };
        ...
        let init_h = if self.inputs.len() > 5 && !self.inputs[5].is_empty() {
            get_tensor(values, &self.inputs[5])?.floats().context("in Lstm layer")?
        } else {
            &zero_h
        };
        ...
        let init_c = if self.inputs.len() > 6 && !self.inputs[6].is_empty() {
            get_tensor(values, &self.inputs[6])?.floats().context("in Lstm layer")?
        } else {
            &zero_h
        };
