    ...
    fn execute(&mut self, values: &HashMap<String, Tensor>, output: &mut Tensor) -> Result<()> {
        ...
        for val in buf.iter_mut() {
            ...
            let va: f64 = match a.dtype() {
                DType::Float => a.floats().context("in Less layer")?[ai] as f64,
                DType::Int64 => a.ints().context("in Less layer")?[ai] as f64,
                DType::String => unreachable!("strings not supported"),
            };
            let vb: f64 = match b.dtype() {
                DType::Float => b.floats().context("in Less layer")?[bi] as f64,
                DType::Int64 => b.ints().context("in Less layer")?[bi] as f64,
                DType::String => unreachable!("strings not supported"),
            };
