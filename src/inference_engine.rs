...
impl InferenceEngine {
    ...
    fn load_plan_values(inputs: &mut HashMap<String, Tensor>, plan: &Plan) {
    ...
    fn ensure_plan(&mut self) -> Result<()> {
        // Check if current plan already matches
        ...
        // Search cache for a matching plan
        ...
        // Build input sizes map only when we need a new plan
    ...
    /// Switch self.inputs to use the plan at the given cache index.
    fn switch_to_plan(&mut self, idx: usize) {
        // Preserve user input tensors
        let saved_inputs: Vec<(String, Tensor)> = self
            .input_names
            .iter()
            .filter_map(|n| self.inputs.remove_entry(n))
            .collect();

        self.inputs.clear();
        Self::load_plan_values(&mut self.inputs, &self.plan_cache[idx]);
    ...
    pub fn run_planned(&mut self) -> Result<&HashMap<String, Tensor>> {
        ...
        let result = self.run_for_inner(&output_names, &mut outputs);
    ...
    pub fn run_for(
        ...
        self.ensure_plan()?;
        self.run_for_inner(output_names, outputs)
    }
        ...
        for node in &mut plan.nodes {
            match node {
                PlanNode::Single { output, layer } => {
                    ...
                    layer.execute(&self.inputs, &mut out)?;
                    self.inputs.insert(key, out);
                }
                PlanNode::Loop(loop_layer) => {
                    loop_layer.execute(&mut self.inputs)?;
                }
                PlanNode::Split(split_layer) => {
                    split_layer.execute(&mut self.inputs)?;
                }
                PlanNode::If(if_layer) => {
                    if_layer.execute(&mut self.inputs)?;
                }
                PlanNode::TopK(topk_layer) => {
                    topk_layer.execute(&mut self.inputs)?;
                }
                PlanNode::Scan(scan_layer) => {
                    scan_layer.execute(&mut self.inputs)?;
                }
                #[cfg(feature = "xnnpack")]
                PlanNode::XnnpackSubgraph(sg) => {
                    sg.execute(&mut self.inputs)?;
                }
            }
        }

        for name in output_names {
            if let Some(src) = self.inputs.get(name) {
